    ...
    <div style={{ minHeight: '100vh', background: '#03040a', color: '#f0f4ff', fontFamily: 'var(--font-space-grotesk), system-ui, sans-serif' }}>
      ...
      <nav style={{ borderBottom: '1px solid rgba(255,255,255,0.06)', padding: '0 32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 56 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 32 }}>
          ...
          <a href="/dashboard/watchlist" style={{ fontSize: 12, color: '#00ff88', textDecoration: 'none', letterSpacing: '0.08em' }}>Watchlist</a>
      ...
      <div style={{ maxWidth: 1000, margin: '0 auto', padding: '48px 32px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 32, flexWrap: 'wrap', gap: 16 }}>
          <div>
            <div style={{ fontFamily: 'var(--font-jetbrains-mono)', fontSize: 11, letterSpacing: '0.2em', color: '#3d4a5c', marginBottom: 4, textTransform: 'uppercase' as const }}>Compliance</div>
            <h1 style={{ fontFamily: 'var(--font-space-grotesk), system-ui, sans-serif', fontSize: 24, fontWeight: 700, color: '#f0f4ff', margin: 0 }}>Watchlist</h1>
          </div>
          <div style={{ fontFamily: 'var(--font-jetbrains-mono)', fontSize: 10, color: '#3d4a5c', letterSpacing: '0.1em' }}>
            Checked daily at 9am UTC · alerts via email
          </div>
        </div>

        {/* Add form */}
        <form onSubmit={handleAdd} style={{ background: '#080b14', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 4, padding: '20px 24px', marginBottom: 24 }}>
          <div style={{ fontFamily: 'var(--font-jetbrains-mono)', fontSize: 10, letterSpacing: '0.15em', color: '#8892a4', marginBottom: 16, textTransform: 'uppercase' as const }}>Add Address</div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr auto auto 200px auto', gap: 16, alignItems: 'end', flexWrap: 'wrap' }}>
            <div>
              <label style={{ display: 'block', fontSize: 10, letterSpacing: '0.12em', color: '#3d4a5c', marginBottom: 6 }}>ADDRESS *</label>
              <input
                type="text"
                value={addAddress}
            ...
            <div>
              <label style={{ display: 'block', fontSize: 10, letterSpacing: '0.12em', color: '#3d4a5c', marginBottom: 6 }}>CHAIN</label>
              <div style={{ display: 'flex', gap: 4 }}>
                {(['ETH', 'BTC', 'TRX'] as Chain[]).map(c => (
                  <button
                    ...
                    style={{
                      padding: '6px 10px',
                      background: addChain === c ? 'rgba(0,255,136,0.1)' : 'transparent',
                      border: `1px solid ${addChain === c ? 'rgba(0,255,136,0.3)' : 'rgba(255,255,255,0.08)'}`,
                      borderRadius: 2,
                      color: addChain === c ? '#00ff88' : '#8892a4',
                      fontSize: 10,
            ...
            <div style={{ width: 200 }}>
              <label style={{ display: 'block', fontSize: 10, letterSpacing: '0.12em', color: '#3d4a5c', marginBottom: 6 }}>LABEL (OPTIONAL)</label>
              <input
                type="text"
                value={addLabel}
            ...
            <div>
              <button
                ...
                style={{
                  padding: '10px 20px',
                  background: addAddress.trim() && !adding ? 'rgba(0,255,136,0.1)' : 'rgba(255,255,255,0.03)',
                  border: `1px solid ${addAddress.trim() && !adding ? 'rgba(0,255,136,0.3)' : 'rgba(255,255,255,0.06)'}`,
                  borderRadius: 4,
                  color: addAddress.trim() && !adding ? '#00ff88' : '#3d4a5c',
                  fontSize: 11,
                  letterSpacing: '0.12em',
        ...
        {error ? (
          ...
          <div style={{ padding: 40, textAlign: 'center', color: '#3d4a5c', fontSize: 13 }}>Loading...</div>
        ) : entries.length === 0 ? (
          <div style={{ padding: '48px', textAlign: 'center', color: '#3d4a5c', fontSize: 13, background: '#080b14', borderRadius: 4, border: '1px solid rgba(255,255,255,0.06)' }}>
              ...
              <thead>
                <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                  {['Label', 'Address', 'Chain', 'Risk Level', 'Score', 'Last Checked', ''].map(h => (
                    <th key={h} style={{ padding: '10px 16px', textAlign: 'left', fontSize: 10, letterSpacing: '0.12em', color: '#3d4a5c', fontWeight: 600, fontFamily: 'var(--font-jetbrains-mono)' }}>
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {entries.map(entry => (
                  <tr key={entry.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.03)' }}>
                    <td style={{ ...cell, color: '#8892a4', maxWidth: 140, overflow: 'hidden', textOverflow: 'ellipsis' }}>
                      {entry.label ?? <span style={{ color: '#3d4a5c' }}>—</span>}
                    </td>
                    <td style={{ ...cell }}>
                      <a
                    ...
                    <td style={{ ...cell, color: '#8892a4' }}>{entry.chain}</td>
                    <td style={{ ...cell }}>
                      {entry.last_risk_level ? (
                        <span style={{ fontSize: 10, letterSpacing: '0.1em', color: RISK_COLORS[entry.last_risk_level] ?? '#f0f4ff', fontWeight: 700 }}>
                          {entry.last_risk_level}
                        </span>
                      ) : (
                        <span style={{ color: '#3d4a5c', fontSize: 10 }}>NOT CHECKED</span>
                      )}
                    </td>
                    <td style={{ ...cell, color: entry.last_risk_score !== null ? '#f0f4ff' : '#3d4a5c' }}>
                      {entry.last_risk_score ?? '—'}
                    </td>
                    <td style={{ ...cell, color: '#8892a4' }}>
                      {fmtDate(entry.last_checked_at)}
                    </td>
                    <td style={{ padding: '13px 16px' }}>
                      <button
                        onClick={() => handleRemove(entry.id)}
                        style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#3d4a5c', fontSize: 11, fontFamily: 'var(--font-jetbrains-mono)', letterSpacing: '0.08em', padding: 0 }}
                        onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.color = '#ff3b3b'; }}
                        onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.color = '#3d4a5c'; }}
                      >
                        Remove
                      </button>
                    </td>
