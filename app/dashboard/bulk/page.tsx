...
export default function BulkPage() {
  ...
  const btnStyle = (primary: boolean): React.CSSProperties => ({
    padding: '10px 20px',
    background: primary ? 'rgba(6,182,212,0.1)' : '#001824',
    border: primary ? '1px solid rgba(6,182,212,0.3)' : '1px solid rgba(255,255,255,0.08)',
    borderRadius: 4,
    color: primary ? '#06b6d4' : '#7ec8d8',
    fontSize: 11,
    ...
    <div style={{ minHeight: '100vh', background: '#00080f', color: '#ecfeff', fontFamily: 'var(--font-space-grotesk), system-ui, sans-serif' }}>
      ...
      <nav style={{ borderBottom: '1px solid rgba(6,182,212,0.08)', padding: '0 32px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: 56 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 32 }}>
          ...
          <a href="/dashboard/bulk" style={{ fontSize: 12, color: '#06b6d4', textDecoration: 'none', letterSpacing: '0.08em' }}>Bulk Screen</a>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '48px 32px' }}>
        ...
        <div style={{ marginBottom: 36 }}>
          <div style={{ fontFamily: 'var(--font-jetbrains-mono)', fontSize: 11, letterSpacing: '0.2em', color: '#1e4d5c', marginBottom: 8, textTransform: 'uppercase' as const }}>Compliance</div>
          <h1 style={{ fontFamily: 'var(--font-space-grotesk), system-ui, sans-serif', fontSize: 28, fontWeight: 700, color: '#ecfeff', margin: '0 0 8px', letterSpacing: '-0.01em' }}>Bulk Address Screening</h1>
          <p style={{ fontFamily: 'var(--font-inter), system-ui, sans-serif', fontSize: 13, color: '#7ec8d8', margin: 0, lineHeight: 1.5 }}>
            Paste addresses one per line, or upload a CSV. Add <code style={{ fontFamily: 'var(--font-jetbrains-mono)', fontSize: 11, color: '#1e4d5c' }}>,CHAIN</code> after each address to override the default chain.
          </p>
        </div>

        {/* Input card */}
        <div style={{ background: '#001824', border: '1px solid rgba(6,182,212,0.08)', borderRadius: 4, padding: '24px', marginBottom: 24 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
            ...
            {(['ETH', 'BTC', 'TRX', 'SOL'] as Chain[]).map(c => (
              <button
                ...
                style={{
                  padding: '5px 14px',
                  background: defaultChain === c ? 'rgba(6,182,212,0.1)' : 'transparent',
                  border: `1px solid ${defaultChain === c ? 'rgba(6,182,212,0.3)' : 'rgba(255,255,255,0.08)'}`,
                  borderRadius: 2,
                  color: defaultChain === c ? '#06b6d4' : '#7ec8d8',
                  fontSize: 10,
                  letterSpacing: '0.12em',
                  cursor: 'pointer',
                  fontFamily: 'var(--font-jetbrains-mono)',
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 14, flexWrap: 'wrap', gap: 12 }}>
            <span style={{ fontFamily: 'var(--font-jetbrains-mono)', fontSize: 10, color: parsed.length > 0 ? '#7ec8d8' : '#1e4d5c', letterSpacing: '0.08em' }}>
            ...
            <button
              ...
              style={{
                padding: '10px 28px',
                background: parsed.length > 0 && !running ? 'rgba(6,182,212,0.1)' : 'rgba(255,255,255,0.03)',
                border: `1px solid ${parsed.length > 0 && !running ? 'rgba(6,182,212,0.3)' : 'rgba(6,182,212,0.08)'}`,
                borderRadius: 4,
                color: parsed.length > 0 && !running ? '#06b6d4' : '#1e4d5c',
                fontSize: 11,
        ...
        {rows.length === 0 && (
          <div style={{ background: '#001824', border: '1px solid rgba(6,182,212,0.08)', borderRadius: 4, padding: '56px 32px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16 }}>
            ...
            <div style={{ fontFamily: 'var(--font-jetbrains-mono)', fontSize: 10, letterSpacing: '0.2em', color: '#1e4d5c', textTransform: 'uppercase' as const }}>
              Paste addresses above to begin
            </div>
            <div style={{ fontSize: 13, color: '#1e4d5c', textAlign: 'center', lineHeight: 1.6, maxWidth: 340 }}>
        ...
        {rows.length > 0 && (
          <div>
            {/* Progress bar */}
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
              <div style={{ flex: 1, height: 2, background: 'rgba(6,182,212,0.05)', borderRadius: 1, overflow: 'hidden' }}>
                <div
                  style={{
                    height: '100%',
                    width: `${progress}%`,
              ...
              <span style={{ fontFamily: 'var(--font-jetbrains-mono)', fontSize: 10, color: '#7ec8d8', letterSpacing: '0.08em', flexShrink: 0 }}>
            ...
            {allDone && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 24, padding: '14px 20px', background: '#001824', border: '1px solid rgba(6,182,212,0.08)', borderRadius: 4, marginBottom: 16, flexWrap: 'wrap' }}>
                <span style={{ fontFamily: 'var(--font-jetbrains-mono)', fontSize: 10, letterSpacing: '0.12em', color: '#7ec8d8' }}>
                  SCREENED <span style={{ color: '#ecfeff', fontWeight: 700 }}>{doneCount}</span>
                </span>
                <span style={{ width: 1, height: 16, background: 'rgba(6,182,212,0.08)' }} />
                <span style={{ fontFamily: 'var(--font-jetbrains-mono)', fontSize: 10, letterSpacing: '0.12em', color: '#7ec8d8' }}>
                  HIGH / CRITICAL{' '}
                  <span style={{ color: highCriticalCount > 0 ? '#ff8c00' : '#ecfeff', fontWeight: 700 }}>{highCriticalCount}</span>
                </span>
                <span style={{ width: 1, height: 16, background: 'rgba(6,182,212,0.08)' }} />
                <span style={{ fontFamily: 'var(--font-jetbrains-mono)', fontSize: 10, letterSpacing: '0.12em', color: '#7ec8d8' }}>
                  CLEAN <span style={{ color: '#06b6d4', fontWeight: 700 }}>{cleanCount}</span>
                </span>
                <div style={{ marginLeft: 'auto' }}>
                  <button
            ...
            {/* Results table */}
            <div style={{ background: '#001824', border: '1px solid rgba(6,182,212,0.08)', borderRadius: 4, overflow: 'hidden' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ borderBottom: '1px solid rgba(6,182,212,0.08)' }}>
                    {['#', 'Address', 'Chain', 'Score', 'Risk Level', 'Top Signal', 'Status'].map(h => (
                      <th key={h} style={{ padding: '10px 14px', textAlign: 'left', fontSize: 10, letterSpacing: '0.12em', color: '#1e4d5c', fontWeight: 600, fontFamily: 'var(--font-jetbrains-mono)' }}>
                        {h}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  {rows.map(row => (
                    <tr key={row.index} style={{ borderBottom: '1px solid rgba(255,255,255,0.03)', opacity: row.status === 'queued' ? 0.35 : 1, transition: 'opacity 0.2s' }}>
                      {/* # */}
                      <td style={{ ...cell, color: '#1e4d5c', width: 40 }}>{row.index + 1}</td>
                      ...
                      <td style={{ ...cell }}>
                        {row.status === 'done' || row.status === 'error' ? (
                          <a
                            ...
                            style={{ color: '#06b6d4', textDecoration: 'none', fontFamily: 'var(--font-jetbrains-mono)', fontSize: 12 }}
                            title={row.address}
                          >
                            {truncateAddr(row.address)}
                          </a>
                        ) : (
                          <span style={{ color: '#7ec8d8' }} title={row.address}>{truncateAddr(row.address)}</span>
                        )}
                      </td>

                      {/* Chain */}
                      <td style={{ ...cell, color: '#7ec8d8', width: 60 }}>{row.chain}</td>

                      {/* Score */}
                      <td style={{ ...cell, width: 60 }}>
                        {row.status === 'done' && row.score !== null ? (
                          <span style={{ color: RISK_COLORS[row.riskLevel ?? ''] ?? '#ecfeff', fontWeight: 700 }}>{row.score}</span>
                        ) : row.status === 'running' ? (
                          <span style={{ color: '#1e4d5c' }}>—</span>
                        ) : (
                          <span style={{ color: '#1e4d5c' }}>—</span>
                        )}
                      </td>
                      ...
                      <td style={{ ...cell, width: 100 }}>
                        {row.status === 'done' && row.riskLevel ? (
                          <span style={{ fontSize: 10, letterSpacing: '0.1em', color: RISK_COLORS[row.riskLevel] ?? '#ecfeff', fontWeight: 700 }}>
                            {row.riskLevel}
                          </span>
                        ) : row.status === 'running' ? (
                          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 10, color: '#1e4d5c', letterSpacing: '0.1em' }}>
                            <span style={{
                          ...
                          <span style={{ fontSize: 10, color: '#1e4d5c', letterSpacing: '0.1em' }}>QUEUED</span>
                        ) : null}
                      </td>

                      {/* Top Signal */}
                      <td style={{ ...cell, color: '#7ec8d8', maxWidth: 180, overflow: 'hidden', textOverflow: 'ellipsis' }}>
                        {row.status === 'done' ? (row.topSignal ?? <span style={{ color: '#1e4d5c' }}>—</span>) : <span style={{ color: '#1e4d5c' }}>—</span>}
                      </td>

                      {/* Status */}
                      <td style={{ ...cell, width: 100 }}>
                        {row.status === 'done' && (
                          <span style={{ fontSize: 10, color: '#06b6d4', letterSpacing: '0.08em' }}>DONE</span>
                        )}
                        {row.status === 'queued' && (
                          <span style={{ fontSize: 10, color: '#1e4d5c', letterSpacing: '0.08em' }}>QUEUED</span>
                        )}
                        {row.status === 'running' && (
                          ...
                          >
                            ...
                            {hoveredError === row.index && row.errorMsg && (
                              <span style={{
                                ...
                                background: '#001f2e', border: '1px solid rgba(255,59,59,0.3)',
