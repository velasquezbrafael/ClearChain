...
/**
 * Core fetch helper — wraps a single alchemy_getAssetTransfers JSON-RPC call.
 * Returns an empty array on any failure rather than throwing, so callers can
 * continue with partial data.
 *
 * @param address    Ethereum wallet address
 * @param category   "external" for ETH, "erc20" for token transfers
 */
async function fetchAlchemyTransfers(
  ...
  if (!apiKey) {
    console.error('[ClearChain/alchemy] ALCHEMY_API_KEY env var is not set');
    return [];
  }
  ...
  // We fetch both inbound (toAddress) and outbound (fromAddress) transfers
  // in a single call by omitting address directional filter and relying on
  // the address field being present in either from or to. Alchemy supports
  // this via separate calls; we merge both directions below.
  const body = {
    jsonrpc: '2.0',
    id: 1,
    ...
    params: [
      {
        fromBlock: '0x0',
        toBlock: 'latest',
        // Alchemy requires separate calls for sent and received
        // We use toAddress here; the outbound call uses fromAddress.
        // Callers invoke this helper twice and merge.
        ...(category === 'external'
          ? { category: ['external'] }
          : { category: ['erc20'] }),
        withMetadata: true,
        ...
        order: 'desc',
        // Include the queried address on both sides
        fromAddress: address,
        toAddress: address,
      },
    ],
  ...
  // Note: Alchemy does not support filtering by BOTH fromAddress and toAddress
  // simultaneously in one call (it treats them as AND, not OR). We therefore
  // make two separate calls per category below in the public functions.
  ...
  try {
    ...
    if (data.error) {
      console.error(`[ClearChain/alchemy] RPC error ${data.error.code}: ${data.error.message}`);
      return [];
    }

    return data.result?.transfers ?? [];
  } catch (err) {
    console.error('[ClearChain/alchemy] Fetch failed:', err);
    return [];
  }
}
